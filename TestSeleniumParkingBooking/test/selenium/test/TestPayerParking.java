...
public class TestPayerParking {
	...
	/**
	 * Test to open the web page, assert that the page title click on payer
	 * parking
	 */
	@Test
	public void testPayerParking() {
		driver.get(Messages.getString("project_address"));
		assertEquals(Messages.getString("project_homepage_title"), driver.getTitle());
		try {
			driver.findElement(By.xpath("//a[@href='payerParking.jsp']")).click();
			Thread.sleep(9000);
	 ...
	 */
	@Test
	public void testPayerParkingRemplirChamps() {
		driver.get(Messages.getString("project_address"));
		assertEquals(Messages.getString("project_homepage_title"), driver.getTitle());
		try {
			attendre(200);
